...
if (isset($_POST['btnSave'])) {
  ...
  if (!isset($_SESSION['Sale_Functions']) || count($_SESSION['Sale_Functions']) === 0) {
      echo "<script>
              alert('Error: No products added to the sale. Please add at least one product before saving.');
              window.location='createSale.php';
            </script>";
      exit();
  }
  ...
  $cboCustomerID = $_POST['cboCustomerID'];
  ...
  $cboWarehouseID = $_POST['cboWarehouseID'];
  $cboStatus = $_POST['cboStatus'];

  // Check if SaleID already exists
  $checkQuery = "SELECT * FROM sale WHERE SaleID = '$txtSaleID'";
  $checkResult = mysqli_query($connection, $checkQuery);

  if (mysqli_num_rows($checkResult) > 0) {
      echo "<script>window.alert('Error: Duplicate SaleID.')</script>";
  } else {
      // **Validate Product Quantities Before Insert**
      $size = count($_SESSION['Sale_Functions']);
      $validQuantities = true;

      for ($b = 0; $b < $size; $b++) {
          $ProductID = $_SESSION['Sale_Functions'][$b]['ProductID'];
          $SaleQuantity = $_SESSION['Sale_Functions'][$b]['SaleQuantity'];
          ...
          if ($ret) {
              $arr = mysqli_fetch_array($ret);
              $CurrentQuantity = $arr['Quantity'];

              // Check if sufficient quantity is available
              if ($CurrentQuantity < $SaleQuantity) {
                  echo "<script>
                          alert('Error: Insufficient quantity for ProductID: $ProductID. Available: $CurrentQuantity, Requested: $SaleQuantity.');
                          window.location='createSale.php';
                        </script>";
                  $validQuantities = false;
                  break; // Stop checking further if any product is insufficient
              }
          } else {
              echo "<p>Error fetching product data for ProductID: $ProductID. " . mysqli_error($connection) . "</p>";
              $validQuantities = false;
              break;
          }
      }

      // If all quantities are valid, proceed with sale insertion
      if ($validQuantities) {
          $Insert1 = "INSERT INTO `sale`
                      (`SaleID`, `SaleDate`, `CustomerID`, `EmployeeID`, `TotalQuantity`, `TotalAmount`, `VAT`, `GrandTotal`, `Status`, `Note`, `WarehouseID`) 
                      VALUES
                      ('$txtSaleID','$txtSaleDate','$cboCustomerID','$EmployeeID','$txtTotalQuantity','$txtTotalAmount','$txtVAT','$txtGrandTotal','$cboStatus','$txtNote','$cboWarehouseID')";
          ...
          if ($ret1) {
              ...
              for ($b = 0; $b < $size; $b++) {
                  ...
                  if ($ret) {
                      ...
                      $updateQuery = "UPDATE product SET Quantity = '$NewQuantity' WHERE ProductID = '$ProductID'";
                      $updateResult = mysqli_query($connection, $updateQuery);

                      if (!$updateResult) {
                          echo "<p>Error updating product quantity for ProductID: $ProductID. " . mysqli_error($connection) . "</p>";
                      }

                      // Insert into saledetail table
                      $Insert2 = "INSERT INTO `saledetail`
                                  (`SaleID`, `ProductID`, `Quantity`) 
                                  VALUES
                                  ('$txtSaleID', '$ProductID', '$SaleQuantity')";
              ...
              if ($ret2) {
                  ...
                  echo "<p>Something went wrong in saledetail insert: " . mysqli_error($connection) . "</p>";
              }
          } else {
...
if(isset($_POST['btnAdd'])) 
{
	...
	$SaleQuantity=$_POST['txtQuantity'];

	AddProduct($ProductID, $SaleQuantity);
}
?>
<!DOCTYPE html>
<html lang="en">
  <head>
    ...
    <title>Aung Bi Win Rice Trading House - Create Sale</title>
    ...
    <link href="css/sb-admin-2.min.css?<?php echo time(); ?>" rel="stylesheet" />
  </head>
                  ...
                  <div class="card">
                    <div class="card-body">
                      <div class="row mb-4">
                        ...
                        <div class="col-lg-3 col-sm-6 col-12">
                          <div class="form-group">
                            ...
                            <input
                              ...
                              id="txtSaleID"
                              name="txtSaleID"
                              value="<?php echo AutoID('sale','SaleID','S-',6) ?>"
                              readonly
                            />
                          </div>
                        </div>
                        ...
                        <div class="col-lg-3 col-sm-6 col-12">
                          <div class="form-group">
                            <label for="saleDate">Slae Date</label>
                            <div class="input-group">
                              <input
                                type="text"
                          ...
                          <table class="table table-striped">
                            <thead>
                              <tr>
                                ...
                                <th>Qutnatity(bag)</th>
                                <th>Sale Price</th>
                                <th class="text-end">Sub Total(MMK)</th>
                                <th></th>
                              </tr>
                            </thead>
                            <tbody>
                            ...
                            if(!isset($_SESSION['Sale_Functions'])) 
                            {
                              ...
                              for($x=0;$x<$size;$x++) 
                              { 
                                $ProductID=$_SESSION['Sale_Functions'][$x]['ProductID'];

                                echo "<tr>";
                                  echo "<td>"	. $_SESSION['Sale_Functions'][$x]['ProductName'] ."</td>";
                                  echo "<td>"	. $_SESSION['Sale_Functions'][$x]['SaleQuantity'] ."</td>";
                                  echo "<td>"	. $_SESSION['Sale_Functions'][$x]['ProductPrice'] ."</td>";
                                  echo "<td>" . $_SESSION['Sale_Functions'][$x]['ProductPrice'] * 
                                                $_SESSION['Sale_Functions'][$x]['SaleQuantity']. "</td>";
                                  echo "<td><a class='delete-set' href='createSale.php?action=remove&ProductID=$ProductID'><img src='img/delete.svg' alt='delete'/></a></td>";
                                echo "</tr>";
                              }
                            }
                              ?>
                            ...
                            <table class="table table-bordered table-striped">
                              ...
                              <tbody>
                                ...
                                <tr>
                                  ...
                                  <td><input type="text" class="form-control-plaintext" name="txtVAT" value="<?php echo CalculateTotalAmount() * 0.01; ?>" readonly> MMK</td>
                                </tr>
                                <tr>
                                  <td><strong>Grand Total</strong></td>
                                  <td><input type="text" class="form-control-plaintext" name="txtGrandTotal" value="<?php echo CalculateTotalAmount() + CalculateTotalAmount() * 0.01; ?>" readonly> MMK</td>
                                </tr>
                              </tbody>
                            </table>
                          </div>
                      ...
                      <div class="row">
                        ...
                        <div class="col-lg-3 col-sm-6 col-12">
                          <div class="form-group">
                            ...
                            <div class="d-flex">
                              <select class="form-control" name="cboCustomerID" >
                                <option value="" selected disabled>Choose Customer</option>
                                    ...
                                    echo "<option value='$CustomerID'>" . $arr['CustomerName'] . "</option>";
                                    }
                                    ?>
                              </select>
                            </div>
                        ...
                        <div class="col-lg-3 col-sm-6 col-12">
                          <div class="form-group">
                            ...
                            <div class="d-flex">
                              <select class="form-control" name="cboWarehouseID" >
                        ...
                        <div class="col-lg-12">
                          ...
                          <a href="createSale.php?action=clearall" class="btn btn-light"
                            >Cancel</a
                          >
                        </div>
                      </div>
