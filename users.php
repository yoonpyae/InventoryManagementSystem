...
if (isset($_POST['btnSave'])) {
  ...
  if (!preg_match($passwordPattern, $txtPassword)) {
      echo "<script>window.alert('Password must be at least 8 characters long and include at least one uppercase letter, one number, and one special character.')</script>";
      echo "<script>window.location='users.php'</script>";
      exit(); // Stop further execution if password is invalid
  }

  // Image Upload
  $fileProfilePic = $_FILES['fileProfilePic']['name']; // alex.jpg
  $folder = "UserPhoto/";                              // UserPhoto/
  $FileName = $folder . "_" . $fileProfilePic;         // UserPhoto/_alex.jpg
  $copied = copy($_FILES['fileProfilePic']['tmp_name'], $FileName);
  ...
  $checkEmail = "SELECT * FROM employee WHERE Email='$txtEmail'";
  ...
  if ($size > 0) {
      ...
      $hashedPassword = password_hash($txtPassword, PASSWORD_BCRYPT);

      $Insert = "INSERT INTO `employee`
          (`EmployeeName`, `Position`, `Address`, `PhoneNumber`, `Email`, `Username`, `Password`, `ProfilePicture`) 
          VALUES
          ('$txtName','$txtPosition','$txtAddress','$txtPhoneNumber','$txtEmail','$txtUsername','$hashedPassword','$FileName')";
...
if (isset($_POST['btnUpdate'])) {
    ...
    $editName = $_POST['editEmployeeName'];
    ...
    $Update = "UPDATE employee 
               SET EmployeeName='$editName', 
                   Position='$editPosition', 
                   Address='$editAddress', 
                   PhoneNumber='$editPhoneNumber', 
                   Email='$editEmail', 
                   Username='$editUsername' 
               WHERE EmployeeID='$editEmployeeID'";

    $ret = mysqli_query($connection, $Update);

    if ($ret) {
        echo "<script>window.alert('Employee Updated Successfully!')</script>";
        echo "<script>window.location='users.php'</script>";
    } else {
        echo "<p>Something went wrong in updating user: " . mysqli_error($connection) . "</p>";
    }
}
?>
...
<html lang="en">
  <head>
    ...
    <title>Aung Bi Win Rice Trading House - Users</title>
                  ...
                  <div class="card-body">
                    <div class="table-top">
                      ...
                      <div class="d-flex flex-column">
                        ...
                        <a href="users.php" class="btn btn-secondary ">X</a>
                            <form action="users.php" method="get" class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
                              <div class="input-group">
                                <input
                                  type="text"
                                  name="search"
                    ...
                    <?php  
                      ...
                      if($searchTerm) {
                        $query = "SELECT * FROM employee WHERE EmployeeID LIKE '%$searchTerm%' OR EmployeeName LIKE '%$searchTerm%'";
                      } else {
                          $query = "SELECT * FROM employee"; // Show all records if no search term
                      }
                      ...
                      <table class="table datanew">
                        ...
                        { 
                          ...
                          echo "<tr>";
                            ...
                            echo "<td>
                            <a class='me-3 edit-btn' href='#' 
                               data-id='" . $arr['EmployeeID'] . "' 
                               data-name='" . $arr['EmployeeName'] . "' 
                               data-position='" . $arr['Position'] . "' 
                               data-address='" . $arr['Address'] . "' 
                               data-phone='" . $arr['PhoneNumber'] . "' 
                               data-email='" . $arr['Email'] . "' 
                               data-username='" . $arr['Username'] . "'>
                              <img src='img/edit.svg' alt='edit icon' />
                            </a>
                             
                            <a class='me-3 delete-btn' href='deleteUser.php?EmployeeID=$EmployeeID' >
